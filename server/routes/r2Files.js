...
router.post('/r2/presigned-upload-url', authMiddleware, async (req, res) => {
    try {
        ...
        const allowedVisibilityScopes = ['hospital_internal', 'doctor_patient'];
        const allowedConsentStatuses = ['approved', 'pending', 'revoked'];

        if (!allowedVisibilityScopes.includes(visibilityScope)) {
            return res.status(400).json({
                success: false,
                code: 'INVALID_VISIBILITY_SCOPE',
                message: 'Invalid visibility scope.'
            });
        }

        if (!allowedConsentStatuses.includes(consentStatus)) {
            return res.status(400).json({
                success: false,
        ...
        // Doctors must have a live patient access session; do not trust
        // a client-supplied consent status for doctor uploads.
        const effectiveConsentStatus = role === 'doctor' ? 'approved' : consentStatus;
        const effectiveVisibilityScope = role === 'doctor' ? 'doctor_patient' : visibilityScope;
        ...
        if (!user || user.status !== 'active' || user.role !== role) {
            return res.status(403).json({
                success: false,
                code: 'R2_UPLOAD_DENIED',
                message: 'Active authenticated user required.'
            });
        }

        let targetTenant = user.hospital_id;
        ...
        if (role === 'super_admin') {
            ...
            if (!targetTenant) {
                return res.status(400).json({
                    ...
                    message: 'hospitalId is required for super administrator uploads.'
                });
            }
            ...
            if (!patient || !patient.hospital_id) {
                return res.status(403).json({
                    ...
                    message: 'Registered patient profile with hospital context required.'
                });
            }

            if (patientId && String(patientId) !== String(patient.id)) {
                return res.status(403).json({
                    success: false,
                    code: 'PATIENT_OWNERSHIP_DENIED',
                    message: 'Patients can only upload files for their own profile.'
                });
            }

            if (hospitalId && String(hospitalId) !== String(patient.hospital_id)) {
                return res.status(403).json({
                    success: false,
                    code: 'CROSS_HOSPITAL_ACCESS_DENIED',
                    message: 'You cannot upload files to another hospital.'
                });
            }

            targetTenant = patient.hospital_id;
            ...
            if (hospitalId && String(hospitalId) !== String(doctor.hospital_id)) {
                return res.status(403).json({
                    success: false,
                    code: 'CROSS_HOSPITAL_ACCESS_DENIED',
                    message: 'You cannot upload files to another hospital.'
            ...
            if (doctorId && String(doctorId) !== String(doctor.id) && String(doctorId) !== String(userId)) {
                return res.status(403).json({
                    success: false,
                    code: 'DOCTOR_OWNERSHIP_DENIED',
                    message: 'You can only upload files as yourself.'
                });
            }

            targetTenant = doctor.hospital_id;
            targetDoctor = doctor.id;
            targetPatient = patientId || null;
            ...
            if (!targetPatient) {
                return res.status(400).json({
                    ...
                    message: 'A valid patient must be selected when uploading a document as a doctor.'
                });
            }

            const doctorPatientSession = await getDb(
                `SELECT aas.id
                 FROM active_access_sessions aas
                 JOIN patients p ON p.id = aas.patient_id
                 WHERE aas.doctor_id = ?
                   ...
                   AND aas.expires_at > CURRENT_TIMESTAMP
                 LIMIT 1`,
                [doctor.id, targetPatient, doctor.hospital_id]
            );

            if (!doctorPatientSession) {
                return res.status(403).json({
                    success: false,
                    code: 'PATIENT_ACCESS_REQUIRED',
                    message: 'Active patient access approval is required to upload a document.'
                });
            }
            ...
            if (hospitalId && String(hospitalId) !== String(user.hospital_id)) {
                return res.status(403).json({
                    success: false,
                    code: 'CROSS_HOSPITAL_ACCESS_DENIED',
                    message: 'You cannot upload files to another hospital.'
            ...
            return res.status(403).json({
                success: false,
                code: 'R2_UPLOAD_DENIED',
                message: 'This role is not authorized to upload files.'
            });
        }
        ...
        if (!targetTenant) {
            return res.status(403).json({
                success: false,
                code: 'HOSPITAL_CONTEXT_REQUIRED',
                message: 'Hospital context is required for file uploads.'
            });
        }

        if (targetPatient) {
            const patientExists = await getDb(
                `SELECT id
                 FROM patients
                 WHERE id = ? AND hospital_id = ?
                 LIMIT 1`,
                [targetPatient, targetTenant]
            );

            if (!patientExists) {
                return res.status(403).json({
                    success: false,
                    code: 'PATIENT_CONTEXT_INVALID',
                    message: 'The selected patient does not belong to this hospital.'
                });
            }
        }

        if (targetDoctor) {
            const doctorExists = await getDb(
                `SELECT id
                 FROM doctors
                 WHERE (id = ? OR user_id = ?) AND hospital_id = ?
                 LIMIT 1`,
                [targetDoctor, targetDoctor, targetTenant]
            );

            if (!doctorExists) {
                return res.status(403).json({
                    success: false,
                    code: 'DOCTOR_CONTEXT_INVALID',
                    message: 'The selected doctor does not belong to this hospital.'
                });
            }

            targetDoctor = doctorExists.id;
        }
        ...
        const fileMetadata = {
            ...
            hospitalId: targetTenant,
            departmentId,
            visibilityScope: effectiveVisibilityScope,
        ...
        try {
            await runDb(
                `INSERT INTO documents (
                    id, file_id, patient_id, doctor_id, uploaded_by, uploaded_for,
                    document_type, file_name, mime_type, file_size,
                    storage_provider, storage_key, bucket_name, hospital_id, department_id,
            ...
            console.error('[Documents Insert Error]:', dbErr);
...
router.post('/r2/confirm-upload', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            ...
            console.error('[R2 Confirm Upload Update Error]:', e);
            return res.status(500).json({
                success: false,
                message: 'Failed to activate document metadata'
            });
        }
    ...
    try {
        ...
        if (meta) {
            ...
            try {
                ...
                console.error('[R2 Delete Metadata Update Error]:', dbErr);
                return res.status(500).json({
                    success: false,
                    code: 'R2_DELETE_METADATA_UPDATE_FAILED',
                    message: 'File was removed from object storage but document metadata could not be updated.'
                });
            }

            if (!meta.hospital_id) {
                console.error('[R2 Delete Error]: Document has no hospital context:', fileId);
                return res.status(500).json({
                    success: false,
                    code: 'R2_DELETE_HOSPITAL_CONTEXT_MISSING',
                    message: 'Document hospital context is missing.'
                });
            }

            try {
                await recordStorageUsage({
                    hospitalId: meta.hospital_id,
                    bytesDelta: -Number(meta.file_size || 0),
                    operationType: 'delete'
                });
            } catch (quotaErr) {
                console.error('[R2 Delete Quota Update Error]:', quotaErr);
                return res.status(500).json({
                    success: false,
                    code: 'R2_DELETE_QUOTA_UPDATE_FAILED',
                    message: 'File was deleted but storage quota could not be updated.'
                });
            }
        }
