...
export default function ClinicalRecordViewer() {
    const { id } = useParams();
    ...
    // Dynamically derive tabs data based on record category and filename
    const getDynamicVitals = () => {
        ...
        // Original dynamic mock fallbacks
        const nameLower = (record?.fileName || '').toLowerCase();
        if (nameLower.includes('sugar') || record?.category === 'Lab Reports') {
            return [
                { label: 'Blood Glucose', value: '142 mg/dL', status: 'elevated', time: '09:15' },
                { label: 'Heart Rate', value: '72 bpm', status: 'normal', time: '08:30' },
                { label: 'SpO2', value: '99%', status: 'normal', time: '08:30' }
            ];
        }
        return [
            { label: 'Blood Pressure', value: '124/80 mmHg', status: 'normal', time: '08:30' },
            { label: 'Heart Rate', value: '70 bpm', status: 'normal', time: '08:30' },
            { label: 'Temperature', value: '36.8 °C', status: 'normal', time: '08:30' },
            { label: 'SpO2', value: '98%', status: 'normal', time: '08:30' }
        ];
    };
    ...
    const getDynamicLabs = () => {
        ...
        if (nameLower.includes('lipid') || nameLower.includes('cholesterol')) {
            return [
                { test: 'Total Cholesterol', result: '220 mg/dL', ref: '< 200', flag: 'HIGH', time: record?.date || 'Recent' },
                { test: 'LDL Cholesterol', result: '135 mg/dL', ref: '< 100', flag: 'HIGH', time: record?.date || 'Recent' },
                { test: 'HDL Cholesterol', result: '42 mg/dL', ref: '> 40', flag: null, time: record?.date || 'Recent' }
            ];
        }
        if (nameLower.includes('sugar') || nameLower.includes('glucose') || nameLower.includes('hba1c')) {
            return [
                { test: 'HbA1c', result: '7.2%', ref: '< 5.7%', flag: 'HIGH', time: record?.date || 'Recent' },
                { test: 'Fasting Glucose', result: '118 mg/dL', ref: '70–99', flag: 'HIGH', time: record?.date || 'Recent' }
            ];
        }
        return [
            { test: 'Hemoglobin', result: '14.2 g/dL', ref: '12–17', flag: null, time: record?.date || 'Recent' },
            { test: 'White Blood Cells', result: '6.5 k/uL', ref: '4.5–11.0', flag: null, time: record?.date || 'Recent' }
        ];
    };
    ...
    const getDynamicPrescriptions = () => {
        ...
        // Original dynamic mock fallbacks
        if (record?.category === 'Prescriptions') {
            return [
                { name: record.fileName || 'Prescription Drug', dose: 'As indicated on clinical file', prescriber: record.doctorName || 'Dr. Eleanor Vance', date: record.date || 'Recent', status: 'Active' }
            ];
        }
        return [
            { name: 'Metformin 500mg', dose: 'Twice daily with meals', prescriber: 'Dr. Eleanor Vance', date: '2026-05-10', status: 'Active' }
        ];
    };
    ...
    if (!id) {
        return (
            <div className="text-center py-20 bg-[#111827] border border-[#1E2D4580] rounded-2xl max-w-lg mx-auto">
                ...
                <button onClick={() => navigate('/dashboard/doctor/records')} className="mt-4 px-4 py-2 bg-[#00C8D4] text-[#0B0F1A] rounded-xl font-bold">
