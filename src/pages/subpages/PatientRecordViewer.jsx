...
import {
    ArrowLeft, ShieldCheck, User, FileText, Activity,
    FlaskConical, Scan, Pill, StickyNote, History,
    Download, Bookmark, DatabaseZap, Eye, Loader2, Play, Lock, Sun, RefreshCw, ZoomIn
} from 'lucide-react';
...
export default function PatientRecordViewer() {
    ...
    // Fetch Record Metadata from Firestore
    useEffect(() => {
        if (!id) {
            setRecord(null);
            setPatient(null);
    ...
    const resolveRecordFileUrl = (rec) => {
        if (!rec) return '';
        let url = rec.fileUrl || '';
        if (!url || url.includes('healthchain.internal') || url === '#') {
            if (rec.cidHash && rec.cidHash.startsWith('Qm') && rec.cidHash !== 'QmXoypizjW3WknFixtdKLw62vVJcH1RHA8b') {
                return `https://ipfs.io/ipfs/${rec.cidHash}`;
            }
            const isPdf = (rec.fileType || '').includes('pdf') || (rec.fileName || '').toLowerCase().endsWith('.pdf');
            return isPdf
                ? 'https://ipfs.io/ipfs/QmSgvgwxZGaFAcxya2Sc37EDbfgPZ2m1SDBMNoB6cEMC3t'
        ...
        try {
            ...
            try {
                await accessRequestService.logAuditActivity('RECORD_DECRYPTED_VIEWED', user.uid, {
                    ...
                    action: 'Patient viewed decrypted record'
                });
            } catch (auditErr) {
    ...
    if (loadingRecord) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-3">
                ...
                <p className="text-sm text-[#8899AA] font-mono">Syncing secure block metadata...</p>
            </div>
        );
    }
    ...
    if (!id) {
        return (
            <div className="text-center py-20 bg-[#111827] border border-[#1E2D4580] rounded-2xl max-w-lg mx-auto">
                ...
                <p className="text-xs text-[#8899AA] mt-1">Please select a record from the Medical Records page to view details.</p>
                <button onClick={() => navigate('/dashboard/patient/records')} className="mt-4 px-4 py-2 bg-[#00C8D4] text-[#0B0F1A] rounded-xl font-bold">
    ...
    if (!record) {
        return (
            <div className="text-center py-20 bg-[#111827] border border-[#1E2D4580] rounded-2xl max-w-lg mx-auto">
                <ShieldCheck className="w-12 h-12 text-[#4A5568] mx-auto mb-3" />
        ...
        <div className="max-w-7xl mx-auto pb-12 flex flex-col h-[calc(100vh-120px)]">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 className="text-2xl font-display font-bold text-white">{record.fileName}</h2>
                            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-wider">
                                <ShieldCheck className="w-3 h-3 animate-pulse" /> Cryptographically Secured
                            </span>
                        </div>
                        <p className="text-sm text-[#8899AA] mt-0.5">{record.category} · Hash: {record.id.slice(0, 8)} · Uploaded: {new Date(record.createdAt?.seconds * 1000 || Date.now()).toLocaleDateString()}</p>
                    </div>
                </div>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-5 min-h-0 overflow-hidden">
                ...
                <div className="lg:col-span-1 flex flex-col gap-4 overflow-y-auto">
                    {/* Record Info Card */}
                    <div className="rounded-2xl bg-[#111827] border border-[#1E2D4580] p-5 flex-shrink-0">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#00C8D4]/20 to-blue-500/20 flex items-center justify-center">
                                <DatabaseZap className="w-6 h-6 text-[#00C8D4]" />
                            </div>
                            <div>
                                <p className="font-display font-bold text-white">Record Ledger</p>
                                <p className="text-xs text-[#8899AA]">On-chain verification</p>
                            </div>
                        </div>
                        <div className="space-y-3 text-xs">
                            <div className="flex flex-col gap-1">
                                <span className="text-[#8899AA]">Patient Wallet</span>
                                <span className="text-white font-mono break-all bg-[#1A2236] p-1.5 rounded border border-[#1E2D4580]">{record.patientId}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="text-[#8899AA]">IPFS Content Hash (CID)</span>
                                <span className="text-[#00C8D4] font-mono break-all bg-[#00C8D4]/5 p-1.5 rounded border border-[#00C8D4]/20">{record.cidHash}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="text-[#8899AA]">SHA-256 Integrity Hash</span>
                                <span className="text-purple-400 font-mono break-all bg-purple-500/5 p-1.5 rounded border border-purple-500/20">{record.rawSha256?.slice(0, 32)}...</span>
                            </div>
                            <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#1E2D4580]">
                                <span className="text-[#8899AA]">Security Layer</span>
                                <span className="text-emerald-400 font-bold uppercase tracking-wider text-[10px] bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">AES-GCM</span>
                            </div>
                        </div>
                    </div>

                    {/* Attending Provider info */}
                    <div className="rounded-2xl bg-[#111827] border border-[#1E2D4580] p-5">
                ...
                <div className="lg:col-span-3 rounded-2xl bg-[#111827] border border-[#1E2D4580] flex flex-col overflow-hidden">
                            ...
                            <div className="flex-1 flex flex-col h-full">
                                {!decryptedUrl ? (
                                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center border border-dashed border-[#1E2D4580] rounded-xl bg-[#111827]/60">
                                        ...
                                        <p className="text-sm text-[#8899AA] max-w-sm mb-6">This file is AES-GCM encrypted client-side. Click Decrypt to download the payload keys and unlock the contents locally.</p>
                                        <button 
                    ...
                    <div className="px-6 py-4 border-t border-[#1E2D4580] bg-[#1A2236]/30 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center gap-2 text-xs text-[#8899AA]">
                            ...
                            <span>Zero-Knowledge Encryption Verified · Decentralized Storage Linked</span>
                        </div>
                    </div>
                </div>
            </div>
